...
{
	// There was a bug here when i was declared as unsigned int. By definition, an unsigned int cannot be < 0;
	// therefore, the below loop was an infinite loop. Further, i would be some massive invalid value for most of it,
	// which would then cause invalid memory accesses => segmentation fault.
	// Fix by letting i become < 0 => change i to signed int.
	int i;
	for (i = size-1; i >= 0; i--) {
